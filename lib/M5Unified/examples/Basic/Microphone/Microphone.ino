...
void setup(void) {
    ...
    // cfg.external_speaker.hat_spk = true;     /// use external speaker (HAT
    // SPK) cfg.external_speaker.hat_spk2 = true;    /// use external speaker
    // (HAT SPK2) cfg.external_speaker.atomic_spk = true;  /// use external
    // speaker (ATOMIC SPK)
        ...
        if (M5.Speaker.isEnabled()) {
            ...
            if (start_pos < record_size) {
                M5.Speaker.playRaw(&rec_data[start_pos],
                                   record_size - start_pos, record_samplerate,
                                   false, 1, 0);
            }
            if (start_pos > 0) {
